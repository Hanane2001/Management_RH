...
<div class="container-fluid">
    <div class="form-card">
        <div class="form-header">
            <h3>
                <i class="fas fa-plus-circle"></i> Add Leave Balance
            </h3>
        ...
        <div class="form-body">
            <form method="POST" action="{{ route('leave-balances.store') }}">
                ...
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-input @error('year') is-invalid @enderror" 
                               name="year" value="{{ old('year', date('Y')) }}" required min="2000" max="2030">
                        @error('year')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-input @error('total_days') is-invalid @enderror" 
                               name="total_days" value="{{ old('total_days', 30) }}" required min="1" max="365" id="total_days">
                        @error('total_days')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" class="form-input @error('used_days') is-invalid @enderror" 
                               name="used_days" value="{{ old('used_days', 0) }}" min="0" id="used_days">
                        @error('used_days')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <button type="submit" class="btn-submit">
                    <i class="fas fa-save"></i> Create Leave Balance
                </button>
            </form>
        </div>
    </div>
